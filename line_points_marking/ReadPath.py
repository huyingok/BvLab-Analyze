...
class ReadPath(InitializeInfo):
    ...
    def select_image_path(cls, image_line_edit, self):
        try:
            ...
            # 读取之前保存的路径信息
            initial_path = settings.value("ImportNeuralImagePath", "")
            # 获取多个文件路径
            ...
            if filenames:
                if not cls.contains_chinese(pp(filenames[0]).parent):
                    ...
                    settings.setValue("ImportNeuralImagePath", last_path)
    ...
    def select_swc_path(cls, dir_line_edit, self):
        try:
            ...
            settings = QSettings("MyCompany", "MyApp")
            # 读取之前保存的路径信息
            initial_path = settings.value("ImportNeuralAnnotationPath", "")
            ...
            if path:
                if not cls.contains_chinese(path):
                    settings.setValue("ImportNeuralAnnotationPath", path)
                    # 打印选择的文件路径
    ...
    def select_config_path(cls, config_lineEdit, self):
        try:
            ...
            settings = QSettings("MyCompany", "MyApp")
            # 读取之前保存的路径信息
            initial_path = settings.value("NeuralConfigPath", "")
            ...
            if filename:
                if not cls.contains_chinese(filename):
                    settings.setValue("NeuralConfigPath", str(pp(filename).parent))
                    # 打印选择的文件路径
                    # print(f"Selected dir: {path}")
    ...
    def select_name_path(cls, name_lineEdit, self):
        try:
            ...
            settings = QSettings("MyCompany", "MyApp")
            # 读取之前保存的路径信息
            initial_path = settings.value("NeuralJsonPath", "")
            initial_path = os.path.join(initial_path, "config.json")
            # 获取文件路径
            filename, filetype = QtWidgets.QFileDialog.getOpenFileName(None, "Select File", initial_path,
                                                                       "Json File (*.json)")
            ...
            if filename:
                if not cls.contains_chinese(filename):
                    settings.setValue("NeuralJsonPath", str(pp(filename).parent))
                    # 打印选择的文件路径
                    # print(f"Selected dir: {path}")
    ...
    def select_filter_path(cls, lineedit, self):
        try:
            ...
            settings = QSettings("MyCompany", "MyApp")
            # 读取之前保存的路径信息
            initial_path = settings.value("NeuralFilterPath", "")
            ...
            if path:
                if not cls.contains_chinese(path):
                    settings.setValue("NeuralFilterPath", path)
