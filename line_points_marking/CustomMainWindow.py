...
import cv2
from line_points_marking.ControlStyle import (button_style, tool_bar_style,
...
from line_points_marking.Ui_fun_Form2 import Ui_fun_Form
...
from line_points_marking.Ui_NeuralRevisingWidget import Ui_NeuralRevisingWidget
from line_points_marking.ShortCutDialog import ShortCutDialog
from line_points_marking.Ui_branch_select_Form import Ui_branch_select_Form


class MyWidget(QWidget, Ui_NeuralRevisingWidget, Ui_fun_Form, Ui_branch_select_Form,
               InitializeInfo):
    def __init__(self, parent=None):
    ...
    def create_vtk_view(self):
        ...
        并采用一些优化技术来提高渲染效率，从而使用户能够更流畅地观察到场景的变化'''
        # self.interactor.SetDesiredUpdateRate(0.2)  # vtk体渲染设置帧率
        # self.interactor.SetDesiredUpdateRate(1.0)  # vtk体渲染设置帧率
        # 交互器样式的一种，该样式下，用户是通过控制相机对物体作旋转、Zoom in、缩小等操作
        self.interactor_style = CustomInteractorStyle(self.interactor, self)
        self.interactor_style.SetDefaultRenderer(self.ren)
        self.interactor.SetInteractorStyle(self.interactor_style)
    ...
    def fun_widgets(self):
        ...
        self.adjust_spinBox.hide()
        self.dialog_show_Button.hide()
        self.adjust_label.hide()
        # 灰度
        self.gray_auto_change_button.setStyleSheet(button_style)
        # 点大小
        self.size_spinBox.setStyleSheet(self.qdarkstyle_sheet + double_spin_qdarkstyle)
