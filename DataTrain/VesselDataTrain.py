...
from VesselUnet3D.TrainQThread import VesselDataTrainQThread
from SwcToMask.VesselSwcToMask import VesselSwcToMaskQThread
...
class VesselDataTrain(object):
    def __init__(self, win):
        ...
        # Thread
        # Train
        self.VesselDataTrainQThread = VesselDataTrainQThread(win=self)
        self.VesselDataTrainQThread.finish0.connect(self.data_train_finish)
        self.VesselDataTrainQThread.progress0.connect(self.data_train_progress)
        self.VesselDataTrainQThread.progress1.connect(self.data_train_progress1)
        self.VesselDataTrainQThread.error0.connect(self.data_train_error)
        self.VesselDataTrainQThread.update_modSavePath.connect(self.train_model_update)  # Update latest model path
        # self.VesselDataTrainQThread.update_plot.connect(self.train_plot)  # Training loss curve
        self.VesselDataTrainQThread.preview0.connect(self.train_preview)  # Training result preview
        self.VesselDataTrainQThread.show_preview.connect(self.show_preview_button)
        # Conversion
        self.VesselSwcToMaskQThread = VesselSwcToMaskQThread(win=self)
        self.VesselSwcToMaskQThread.finish0.connect(self.swc_to_mask_finish)
        self.VesselSwcToMaskQThread.warning0.connect(self.swc_to_mask_warning)
    ...
    def train_model_update(self, path):
        ...
        try:
            ...
            self.mess(f"Vessel model update error: {e}", "Prompt", 1)
            self.error_logger(e)
    ...
    def show_TrainResultPreview(self):
        ...
        port = exe_cfg.cfg['Port']['vessel_port']  # Default port is 6008
        host = exe_cfg.cfg['Host']['vessel_host']  # Default host is 127.0.0.1
                        ...
                        if os.path.exists(img_dir):
                            if os.path.exists(swc_dir):
                                ...
                                if res == QMessageBox.Yes:
                                    ...
                                    self.VesselSwcToMaskQThread.start()
                            else:
    ...
    def data_train_finish(self, new_text):
        ...
        self.add_items(text)
        if new_text:
            self.mess(f"{new_text}, cannot train vessel model!", "Prompt", 1)
        else:
            self.mess(f"Vessel model training finished! Time elapsed: {t:.4f} Minute", "Prompt", 1)
    ...
    def data_train_error(self, e):
        ...
        self.add_items(text)
        self.mess(f"Vessel model training error: {e}", "Prompt", 1)
        self.VesselDataTrainQThread.to_stop()
        if self.VesselDataTrainQThread.isRunning():
            self.VesselDataTrainQThread.quit()
            if not self.VesselDataTrainQThread.wait(5000):
                self.VesselDataTrainQThread.terminate()
                self.VesselDataTrainQThread.wait(3000)
        self.recover_control()
    ...
    def data_train_stop(self):
        ...
        if r == QMessageBox.Yes:
            self.train_stop = True
            if self.VesselDataTrainQThread.start_train:
                self.VesselDataTrainQThread.to_stop()
                if self.VesselDataTrainQThread.isRunning():
                    self.VesselDataTrainQThread.quit()
                    if not self.VesselDataTrainQThread.wait(5000):
    ...
    def swc_to_mask_finish(self):
        ...
        if self.train_stop:
            ...
            self.add_items(text)
            self.start_time = time.time()
            self.VesselDataTrainQThread.start()
    ...
    def swc_to_mask_error(self, e):
        self.train_stop = True
        self.mess(f"Vessel label conversion error: {e}", "Prompt", 1)
        self.recover_control()
        ...
        try:
            if look and self.training:
                ...
                if save:
                    ...
                    initial_path = settings.value("VesselTrainPath", "")
                # Get folder path
                ...
                if path:
                    if not self.contains_chinese(path):
                        if save:
                            ...
                            settings.setValue("VesselTrainPath", path)
    ...
    def select_path(self):
        try:
            ...
            settings = QSettings("MyCompany", "MyApp")
            # Read previously saved path
            initial_path = settings.value("VesselTrainConfigPath", "")
