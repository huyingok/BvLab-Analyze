...
from CellUnet3D_DDP.TrainQThread import CellDataTrainQThread
from SwcToMask.CellSwcToMask import CellSwcToMaskQThread
...
class CellDataTrain(object):
    def __init__(self, win):
        ...
        # Thread
        # Train
        self.CellDataTrainQThread = CellDataTrainQThread(win=self)
        self.CellDataTrainQThread.finish0.connect(self.data_train_finish)
        self.CellDataTrainQThread.progress0.connect(self.data_train_progress)
        self.CellDataTrainQThread.progress1.connect(self.data_train_progress1)
        self.CellDataTrainQThread.error0.connect(self.data_train_error)
        self.CellDataTrainQThread.update_modSavePath.connect(self.train_model_update)  # Update latest model path
        self.CellDataTrainQThread.preview0.connect(self.train_preview)  # Training result preview
        self.CellDataTrainQThread.show_preview.connect(self.show_preview_button)
        # Conversion
        self.CellSwcToMaskQThread = CellSwcToMaskQThread(win=self)
        self.CellSwcToMaskQThread.finish0.connect(self.swc_to_mask_finish)
        self.CellSwcToMaskQThread.warning0.connect(self.swc_to_mask_warning)
    ...
    def train_model_update(self, path):
        try:
            ...
            self.mess(f"Cell model update error: {e}", "Prompt", 1)
            self.error_logger(e)
    ...
    def show_TrainResultPreview(self):
        ...
        port = exe_cfg.cfg['Port']['cell_port']  # Default port is 6008
        host = exe_cfg.cfg['Host']['cell_host']  # Default host is 127.0.0.1
                        ...
                        if os.path.exists(img_dir):
                            if os.path.exists(swc_dir):
                                ...
                                if res == QMessageBox.Yes:
                                    ...
                                    self.CellSwcToMaskQThread.start()
                            else:
    ...
    def data_train_finish(self, new_text):
        ...
        self.add_items(text)
        if new_text:
            self.mess(f"{new_text}, cannot train cell model!", "Prompt", 1)
        else:
            self.mess(f"Cell model training finished! Time elapsed: {t:.4f} Minute", "Prompt", 1)
    ...
    def data_train_error(self, e):
        ...
        self.add_items(text)
        self.mess(f"Cell model training error: {e}", "Prompt", 1)
        self.CellDataTrainQThread.to_stop()
        if self.CellDataTrainQThread.isRunning():
            self.CellDataTrainQThread.quit()
            if not self.CellDataTrainQThread.wait(5000):
                self.CellDataTrainQThread.terminate()
                self.CellDataTrainQThread.wait(3000)
        self.recover_control()
    ...
    def data_train_stop(self):
        ...
        if r == QMessageBox.Yes:
            self.train_stop = True
            if self.CellDataTrainQThread.start_train:
                self.CellDataTrainQThread.to_stop()
                if self.CellDataTrainQThread.isRunning():
                    self.CellDataTrainQThread.quit()
                    if not self.CellDataTrainQThread.wait(5000):
    ...
    def swc_to_mask_finish(self):
        ...
        if self.train_stop:
            ...
            self.add_items(text)
            self.start_time = time.time()
            self.CellDataTrainQThread.start()
    ...
    def swc_to_mask_error(self, e):
        self.train_stop = True
        self.mess(f"Cell label conversion error: {e}", "Prompt", 1)
        self.recover_control()
    ...
    def select_dir(self, line_edit, look=0, save=0):
        try:
            if look and self.training:
                ...
                if save:
                    initial_path = settings.value("CellResultsSavePath", "")
                else:
                    initial_path = settings.value("CellTrainPath", "")
                # Get folder path
                path = QFileDialog.getExistingDirectory(None, "Select folder", initial_path)
                if path:
                    if not self.contains_chinese(path):
                        if save:
                            settings.setValue("CellResultsSavePath", path)
                        else:
                            settings.setValue("CellTrainPath", path)
    ...
    def select_path(self):
        try:
            ...
            settings = QSettings("MyCompany", "MyApp")
            # Read previously saved path
            initial_path = settings.value("CellTrainConfigPath", "")
