...
class GetMultiTypeMemoryDataSetAndCropQxz:
    def __init__(self, path, txtName, imgSize, imgPath, maskPath):
        # self.backThre = 0.3
        self.imgSize = imgSize  # xyz
    ...
    def __getitem__(self, ind):
        imgPath = join(self.imgPath, self.nameLs[ind])
        ...
        maskPath = join(self.maskPath, self.nameLs[ind])
        mask = tifffile.imread(maskPath)[:self.imgSize[2], :self.imgSize[1], :self.imgSize[0]] / 255.0
        # mask = np.zeros([self.imgSize[2], self.imgSize[1], self.imgSize[0]], dtype=np.uint8)
