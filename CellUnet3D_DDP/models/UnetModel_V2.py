...
from CellUnet3D_DDP.models.pytorch3dunet.unet3d.buildingblocks import DoubleConv, ResNetBlock, ResNetBlockSE, \
     create_decoders, create_encoders, Encoder, SingleConv
from CellUnet3D_DDP.models.pytorch3dunet.unet3d.utils import get_class, number_of_features_per_level
import torch
...
class AbstractUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, fieldSpace=2, f_maps=64,
                 layer_order='gcr',
        ...
        # Encoder
        # self.Encoder_1 = [Encoder(4, f_maps[0], apply_pooling=False, basic_module=basic_module,
        #     conv_layer_order=layer_order, conv_kernel_size=conv_kernel_size, num_groups=num_groups, padding=conv_padding,
        ...
        for fi in range(2, len(f_maps)):
            self.Encoder_1.append(
                Encoder(f_maps[fi - 1], f_maps[fi], basic_module=basic_module, conv_layer_order=layer_order,
                        conv_kernel_size=conv_kernel_size,
                        num_groups=num_groups, pool_kernel_size=pool_kernel_size, padding=conv_padding,
                        upscale=conv_upscale,
                        dropout_prob=dropout_prob, is3d=is3d)
            )
        self.Encoder_1 = nn.ModuleList(self.Encoder_1)

        # self.Encoder_2 = [Encoder(self.ours_channels, f_maps[0], apply_pooling=False, basic_module=basic_module,
        #     conv_layer_order=layer_order, conv_kernel_size=conv_kernel_size, num_groups=self.ours_channels, padding=conv_padding,
        #     upscale=conv_upscale, dropout_prob=dropout_prob, is3d=is3d)]
        self.Encoder_2 = [
    ...
    def forward(self, x):
        ...
        for i in range(0, self.in_channels):
            j = i * self.fieldSpace
            # print(i, self.sInd, j, j + self.xLen)
        ...
        for encoder1, encoder2, addDec in zip(self.Encoder_1, self.Encoder_2, self.addDec[::-1]):
            x = encoder1(x)
            xx = encoder2(xx)
            # encoders_features.insert(0, addDec(torch.cat([(x[:, ::2] + x[:, 1::2]) / 2, (xx[:, ::2] + xx[:, 1::2]) / 2], dim=1)))
...
if __name__ == '__main__':
    ...
    images = torch.zeros([1, 1, 192, 192, 192], dtype=torch.float32, device=device)
    # with torch.no_grad():
    import time
    t1 = time.time()
    mask = obj1(images)
