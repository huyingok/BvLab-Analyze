            ...
            for kk, (img, mask, name) in enumerate(self.dataLoader):
                ...
                if (iter_count + 1) % 10 == 0:
                    ...
                    print('TRAIN [Epoch %d | %d] [Proce %d | %d] [Loss %.4f]' % (
                        t, turn, kk, len(self.dataLoader), tmpLoss))
                if (iter_count + 1) % self.valCount == 0:
                    ...
                    torch.cuda.empty_cache()
                    self.super_net.train()
                if iter_count % model_savecount == 0:
                    torch.save({'state_dict': self.super_net.state_dict(), 'param': self.optimizer},
                               os.path.join(self.modelPath,
                                            "supernet_%s_%s.pth" % (str(t).zfill(5), str(iter_count).zfill(5))))
                iter_count += 1
                # if kk > 100:
                #     break
