...
class LineImageCutThread(QThread):
                        ...
                        for nz in range(sliceNumber[2]):
                            ...
                            if np.max(new_img) != 0:
                                ...
                                if swc_size:  # Label file is not empty
                                    shape = [start_x, end_x, start_y, end_y, start_z, end_z]
                                    self.SwcCut(shape, cut_swc_path, swcDataLs)
                                else:
    ...
    def SwcCut(self, shape, path, swcDataLs):
        ...
        for swcData in swcDataLs:
            ...
            if dist < r + cut_r:
                for ii, item in enumerate(swcData):
                    ...
                    is_p1 = 0
                    for i in range(3):
                        if np.abs(cut_center[i] - p0[i]) >= np.abs(cut_v[i]):
                            is_p0 += 1
                            break
                        if np.abs(cut_center[i] - p1[i]) >= np.abs(cut_v[i]):
                            is_p1 += 1
                            break
        ...
        for d in swc_result:
            ...
            value = int(d[-1])
            line = f"{d[0]} {0} {(key[0] - xmin):.3f} {(key[1] - ymin):.3f} {(key[2] - zmin):.3f} {rad} {value}\n"
...
def SwcCut(shape, path, swc_path):
    swcData2 = np.loadtxt(swc_path, ndmin=2)
    swcDataLs = SplitSwcData(swcData2)
...
if __name__ == '__main__':
    start = time.time()
    shape = [1485, 27087, 3094, 17523, 4003, 6986]
    save_path = r"D:\SY\10GTestData\splice\cut_swc\neural3.swc"
    swc_path = r"D:\SY\10GTestData\splice\cut_swc\neural.swc"
    SwcCut(shape, save_path, swc_path)
    end = time.time()
    print(end - start)

    root = r"D:\SY\10GTestData\splice\cut_swc\neural3.swc"
    swc_data = np.loadtxt(root, ndmin=2)
    nx, ny, nz = 1485, 3094, 4003
    swc_data2 = []
    for item in swc_data:
        item[2] += nx
        item[3] += ny
        item[4] += nz
        swc_data2.append(item)
    swc_data2 = np.array(swc_data2)
    np.savetxt(r"D:\SY\10GTestData\splice\cut_swc\neural3-1.swc", swc_data2, fmt='%d %d %.2f %.2f %.2f %.4f %d')
