...
if __name__ == '__main__':
    ...
    res = darkObj(imgDict)
    # newImg = res['img'] * 255
    newImg = (res['img'] - res['img'].min()) / (res['img'].max() - res['img'].min()) * 255
    newImg = newImg.astype(np.uint8)
    cv2.imshow('', newImg)
    cv2.waitKey(0)
    # img2 = darkObj.inverse_process(img)[0]
    # cv2.imwrite(r'D:\qxz\MyProject\KKMarkCellBody\Code\NeuronTrack\ICCV_MAET-master\test1.png', img2)
