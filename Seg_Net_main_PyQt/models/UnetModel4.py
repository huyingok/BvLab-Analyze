...
from Seg_Net_main_PyQt.pytorch3dunet.unet3d.buildingblocks import DoubleConv, ResNetBlock, ResNetBlockSE, \
    create_decoders, create_encoders
from Seg_Net_main_PyQt.pytorch3dunet.unet3d.utils import get_class, number_of_features_per_level
import torch
...
class AbstractUNet(nn.Module):
    ...
    def forward(self, x):
        # y = x.unfold(2, 3, 1).unfold(3, 3, 1).unfold(4, 3, 1)
        ...
        for decoder, encoder_features1, encoder_features2, addDec in zip(self.decoders, encoders_features, encoders_features2, self.addDec[1:]):
            ...
            encoder_features = torch.cat([encoder_features1, encoder_features2 * 0.5], dim=1)
