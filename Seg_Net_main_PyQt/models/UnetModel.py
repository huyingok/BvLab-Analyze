...
class AbstractUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        self.fieldSpace = 3
        self.fieldLen = in_channels - 1
        self.batchSize = 1
        self.imgSize = [64, 64, 128]
        self.device = torch.device('cuda:0')
        self.centY = self.imgSize[1] // 2
        self.centX = self.imgSize[0] // 2
        self.input2 = torch.zeros([self.batchSize, in_channels, *self.imgSize[::-1]], dtype=torch.float32).to(
            self.device)
    ...
    def forward(self, x):
        # self.input2[:, :1, ...] = x[:, :1, ...]
        for i in range(self.fieldLen + 1):
            j = i * self.fieldSpace
            # self.input2[:, i + 1, :, :self.centY, :] = input[:, 0, :, j: self.centY + j, :]
            # self.input2[:, i + 1, :, self.centY:, :] = input[:, 0, :, self.centY - j: self.imgSize[1] - j, :]
            self.input2[:, i, :, :self.centY, :self.centX] = x[:, 0, :, j: self.centY + j, j: self.centX + j]
            self.input2[:, i, :, :self.centY, self.centX:] = x[:, 0, :, j: self.centY + j, self.centX - j: self.imgSize[0] - j]
            self.input2[:, i, :, self.centY:, self.centX:] = x[:, 0, :, self.centY - j: self.imgSize[1] - j, self.centX - j: self.imgSize[0] - j]
            self.input2[:, i, :, self.centY:, :self.centX] = x[:, 0, :, self.centY - j: self.imgSize[1] - j, j: self.centX + j]
        # self.input2[:, 1:, ...] = self.input2[:, 1:, ...] / (x[:, [0]] + 0.01)
        self.input2 = self.input2 / (x + 0.01)
        x = self.input2
