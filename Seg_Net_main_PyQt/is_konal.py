...
def dist_grad2():
    ...
    # 设置设备
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # root = r"D:\xueguan\TrainDateSet3\Train2"
    # root = r"D:\xueguan\TrainDateSet2\128"
    root = r"D:\xueguan\TrainDateSet2"
    # mask_dir = os.path.join(root, "mySignal_big")  # Tag
    # mask_dir = os.path.join(root, "mySignal")  # Tag
    # mask_dir = os.path.join(root, "mask")  # Tag
    mask_dir = os.path.join(root, "Predict_Signal")  # Tag
    result_dir = os.path.join(root, "Predict-dist")  # Result
    os.makedirs(result_dir, exist_ok=True)
    images_names = [l for l in os.listdir(mask_dir) if (".tif" in l and ".tiff" not in l)]
    train_masks = []
