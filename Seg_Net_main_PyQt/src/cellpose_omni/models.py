...
# I want to make a 1-class model (cellprob only).
# nclasses=1 is already an option, but much of the code requires 3+ classes.
class CellposeModel(UnetModel):
    ...
    def __init__(self,
        ...
        self.net_type = 'cellpose_residual_{}_style_{}_concatenation_{}{}_abstract_nclasses_{}_nchan_{}_dim_{}'.format(
            ostr[residual_on],
            ostr[style_on],
            ostr[concatenation],
            omnistr[omni],
...
#                            output_device=rank)
    ...
    def eval(self, x, batch_size=8, indices=None, channels=None, channel_axis=None, 
        ...
        # and channels, namely mono channel might have a 2-channel model.
        # I should just check for if the number of channels could possibly match,
        # and warn that internal conversion will happen or may break...
        ...
        if is_dataset:
            ...
            x = transforms.convert_image(x, channels, channel_axis=channel_axis, z_axis=z_axis,  # Image -> 增加维度
            ...
            cellprob, masks, styles = self._run_cp2(x,
                                                    compute_masks=compute_masks,
                                                    normalize=normalize,
                                                    invert=invert,
                                                    rescale=rescale,
            ...
            # (8) binary boundary map
            # 彩色流（Orientation），流场（方向场），距离场，欧拉积分后的像素坐标，Boundary，...，...，二值边界
            # 5-8 were added in Omnipose, hence the unusual placement in the list. 
    ...
    def _run_cp(self, x, compute_masks=True, normalize=True, invert=False,
        ...
        if do_3D:
            ...
            dP = np.zeros((self.dim, nimg,)+s, np.float32)  # 方向场
            cellprob = np.zeros((nimg,)+s, np.float32)  # 细胞概率图
            bounds = np.zeros((nimg,)+s, bool)  # Boundary
    ...
    def _run_cp2(self, x, compute_masks=True, normalize=True, invert=False,
                 rescale=1.0, net_avg=True, resample=True,
                 augment=False, tile=False, tile_overlap=0.1, bsize=224,
                 mask_threshold=0.0, diam_threshold=12., flow_threshold=0.4, niter=None, flow_factor=5.0,
                 min_size=15, max_size=None,
        ...
        if do_3D:
            ...
            bd = np.sum([yf[k][3] for k in range(3)], axis=0) / 3 if self.nclasses == (self.dim + 2) else np.zeros_like(
                cellprob)
            dP = np.stack((yf[1][0] + yf[2][0], yf[0][0] + yf[2][1], yf[0][1] + yf[1][1]), axis=0)  # (dZ, dY, dX)
            if omni:
                dP = np.stack([gaussian_filter(dP[a], sigma=1.5) for a in range(3)])  # remove some artifacts
            ...
            # dP = np.zeros((self.dim, nimg,)+s, np.float32)  # 方向场
            cellprob = np.zeros((nimg,) + s, np.float32)  # 细胞概率图
            masks = np.zeros((nimg,) + s, np.float32)  # Tag
            # bounds = np.zeros((nimg,)+s, bool)  # Boundary
    ...
    def loss_fn(self, lbl, y):
        ...
        # loss = omnipose.core.loss2(self, lbl, y)
