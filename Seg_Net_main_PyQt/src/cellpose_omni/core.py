...
from . import transforms, dynamics, utils, metrics, io, plot
...
from LossPy import EvalScore, LSDLoss
from LossPy2 import EvalDiceFocalLoss, SoftDiceLoss, BCEDiceLoss, DiceFocalLoss
import skimage
...
class UnetModel():
    def __init__(self,
        ...
        if self.torch:
            ...
            # self.nbase = [nchan, 16, 32, 64, 128, 256, 512]
            # self.nbase = [nchan]+[16*(2**i) for i in range(self.nsample)]
    ...
    def _run_net(self, imgs, 
        ...
        imgs, subs = transforms.pad_image_ND(imgs, dim=self.dim)  # 填充图片
        # slices from padding
    ...
    def _run_3D(self, imgs, rsz=1.0, anisotropy=None, net_avg=True, 
        ...
        # imgs = np.squeeze(imgs, axis=0)  # 指定移除第一个维度
    ...
    # import gc
    # 监督学习的核心是通过损失函数来衡量模型的预测结果与真实标签之间的差异
    def _train_step(self, x, lbl):
        ...
        self.net.train()  # must put into train mode——训练模式
        ...
        if self.autocast:
            ...
            y, s = self.net(X)  # 预测结果，y:流场（0，1，2），距离场（3），Boundary（4）;s
            # 在第二个维度插入大小为 1 的维度
            # tensor_unsqueezed = torch.unsqueeze(lbl[:, 0], 1)
            # y, s = self.net(tensor_unsqueezed)  # 预测结果，y:流场（0，1，2），距离场（3），Boundary（4）;s
            with torch.no_grad():
                ny = y.cpu().detach().numpy()
                ny1 = ny[:, self.dim]
                ny2 = ny[:, self.dim + 1]
                for i in range(len(ny)):
                    # masks = self.eval(X[i][0].cpu().numpy())
                    nyi1 = ((ny1[i] - ny1[i].min()) / (ny1[i].max() - ny1[i].min()) * 255).astype(np.uint8)
                    nyi2 = ((ny2[i] - ny2[i].min()) / (ny2[i].max() - ny2[i].min()) * 255).astype(np.uint8)
                    # maski = ((masks[i] - masks[i].min()) / (masks[i].max() - masks[i].min()) * 255).astype(np.uint8)
                    tifffile.imwrite(fr"D:\omnipose-main\results3d\{i+1}\seg_dist_{i+1}.tif", nyi1)
                    tifffile.imwrite(fr"D:\omnipose-main\results3d\{i+1}\seg_bd_{i+1}.tif", nyi2)
                    # tifffile.imwrite(fr"D:\omnipose-main\results3d\{i+1}\seg_mask_{i+1}.tif", maski)
            del X
            loss, raw_loss = self.loss_fn(lbl, y)
            del lbl, y
    ...
    def _train_step2(self, x, lbl):
        ...
        if self.autocast:
            ...
            y, s = self.net(X)  # 预测结果，y:距离场（0）Tag（1）,s
            # 在第二个维度插入大小为 1 的维度
            # tensor_unsqueezed = torch.unsqueeze(lbl[:, 0], 1)
            # y, s = self.net(tensor_unsqueezed)  # 预测结果，y:流场（0，1，2），距离场（3），Boundary（4）;s
            torch.cuda.empty_cache()
            with torch.no_grad():
                ...
                maski = ny[:, 1]
                for i in range(len(ny)):
                    mask1 = ((maski[i] - maski[i].min()) / (maski[i].max() - maski[i].min()) * 255).astype(np.uint8)
                    dist1 = ((disti[i] - disti[i].min()) / (disti[i].max() - disti[i].min()) * 255).astype(np.uint8)
                    tifffile.imwrite(fr"D:\omnipose-main\results3d\{i + 1}\seg_mask_{i + 1}.tif", mask1)
                    tifffile.imwrite(fr"D:\omnipose-main\results3d\{i + 1}\seg_dist_{i + 1}.tif", dist1)
            torch.cuda.empty_cache()
            # loss, raw_loss = self.loss_fn(lbl, y)
            loss, raw_loss = omnipose.core.loss2(self, lbl, y, X)
            del X
            del lbl, y
        # loss.backward(retain_graph=True)  # 多次进行反向传播
        loss.backward()
        self.optimizer.step()
        # 使用 detach 避免内存泄漏
        train_loss = raw_loss.detach() * len(x)  # added detach, probably redundant but trying to fix memory leak
    ...
    def _set_criterion(self):
        ...
        if self.unet:
            ...
            if self.torch:
                ...
                self.loss_criterion = LSDLoss()
                self.precise_dice_loss = SoftDiceLoss()
                self.bce_dice_loss = BCEDiceLoss(0.6, 0.4)
    ...
    def _train_net(self, train_data, train_labels, train_links, test_data=None, test_labels=None,
        ...
        # eval_metric = EvalDiceFocalLoss()
        # eval_metric = DiceLoss()
        val_images = []
        val_masks = []
        if save_path:
            val_img_dir = os.path.join(save_path, "val", "images")
            val_mask_dir = os.path.join(save_path, "val", "masks")
            val_images_names = [l for l in os.listdir(val_img_dir) if (".tif" in l and ".tiff" not in l)]
            with torch.no_grad():
                for ii, val_image_name in enumerate(val_images_names):
                    image_path = os.path.join(val_img_dir, val_image_name)
                    # image = skimage.io.imread(image_path)
                    image = tifffile.imread(image_path)
                    # val_images.append(image)

                    mask_path = os.path.join(val_mask_dir, val_image_name)
                    # mask = skimage.io.imread(mask_path)
                    mask = tifffile.imread(mask_path)
                    mask = ((mask - mask.min()) / (mask.max() - mask.min()) * 1.).astype(np.float32)
                    # val_masks.append(mask)
                    cut_data = [0]
                    cut_label = [0]
                    while np.max(cut_label) <= 0 or np.max(cut_data) <= 0:
                        start_z = np.random.randint(0, image.shape[0] - tyx[0])
                        start_y = np.random.randint(0, image.shape[1] - tyx[1])
                        start_x = np.random.randint(0, image.shape[2] - tyx[2])
                        cut_data = image[
                                   start_z:start_z + tyx[0],
                                   start_y:start_y + tyx[1],
                                   start_x:start_x + tyx[2]
                                   ]
                        cut_label = mask[
                                    start_z:start_z + tyx[0],
                                    start_y:start_y + tyx[1],
                    ...
                    val_images.append(cut_data)
                    val_masks.append(cut_label)
        ...
        if isinstance(learning_rate, (list, np.ndarray)):
            ...
            # scheduler = LearningRateScheduler(self.learning_rate_const, self.n_epochs)
            # LR = scheduler.generate_learning_rates()
            # self.learning_rate = LR

        self.batch_size = batch_size
        self.lr_scheduler = self._set_optimizer(self.learning_rate_const, momentum, weight_decay, SGD)
        # self._set_optimizer(self.learning_rate_const, momentum, weight_decay, SGD)
        self._set_criterion()
        ...
        lastEvalVal = 0
        curEvalVal = 0
        # val_images = self._to_device(val_images)
        # val_images = torch.unsqueeze(val_images, 1)
        val_images = np.expand_dims(val_images, 1)
        val_masks = self._to_device(val_masks)

        val_weights = (1 + (val_masks > 0)) / 2
        val_masks = torch.unsqueeze(val_masks, 1)

        import eikonal
        distances = eikonal.solve_eikonal(val_masks, eps=1e-5, min_steps=200, use_triton=False)
        val_distances = distances[:, 0, :, :, :]
        ...
        for iepoch in range(self.n_epochs):
                ...
                for ibatch in range(0, nimg_per_epoch, batch_size):
                    ...
                    with torch.no_grad():
                        for ind in inds:
                            ...
                            cut_data = [0]
                            while np.max(cut_label) <= 0 or np.max(cut_data) <= 0:
                                start_z = np.random.randint(0, train_labels[ind].shape[0] - tyx_i[0])
                                start_y = np.random.randint(0, train_labels[ind].shape[1] - tyx_i[0])
                                start_x = np.random.randint(0, train_labels[ind].shape[2] - tyx_i[0])
                                cut_data = train_data[ind][:,
                                             start_z:start_z + tyx_i[0],
                                             start_y:start_y + tyx_i[1],
                                             start_x:start_x + tyx_i[2]
                                            ]
                                cut_label = train_labels[ind][
                                             start_z:start_z + tyx_i[0],
                                             start_y:start_y + tyx_i[1],
                                             start_x:start_x + tyx_i[2]
                                            ]
                            # cut_data = gaussian_filter(cut_data, np.random.uniform(0, 2))
                            img_list.append(cut_data)
                            label_list.append(cut_label)
                    imgi = np.stack(img_list, axis=0)
                    lbl = np.stack(label_list, axis=0)
                    del img_list, label_list

                    # 通过随机旋转、Zoom、裁剪和增强操作，生成用于深度学习训练的多样化图像和标签
                    ...
                    #     os.makedirs(fr"D:\omnipose-main\results3d\{i+1}", exist_ok=True)
                    #     tifffile.imwrite(fr"D:\omnipose-main\results3d\{i+1}\imgi_resize{i+1}.tif", imgi[i][0])
                    #     tifffile.imwrite(fr"D:\omnipose-main\results3d\{i+1}\lbl_resize{i+1}.tif", lbl[i])
                    ...
                    # 批量处理分割掩码（masks）并生成流场（flows）: 将分割掩码转换为用于训练的流场表示
                    ...
                    # masks, bd = [torch.stack([x[(Ellipsis,)+slc] for slc in slices]) for x in X]
                    # T, mu = out[2:4]

                    masks, T = omnipose.core.masks_to_flows_batch2(lbl, links,
                                                                   device=self.device,
                                                                   omni=self.omni,
                                                                   dim=self.dim,
                                                                   affinity_field=affinity_field
                    ...
                    with torch.no_grad():
                        for i in range(len(inds)):
                            ...
                            tifffile.imwrite(fr"D:\omnipose-main\results3d\{i+1}\imgi_resize{i+1}.tif", img1)

                            mask1 = masks.detach().cpu().numpy()[i]
                            mask1 = ((mask1 - mask1.min()) / (mask1.max() - mask1.min()) * 255).astype(np.uint8)
                            tifffile.imwrite(fr"D:\omnipose-main\results3d\{i + 1}\mask{i + 1}.tif", mask1)

                            # bd1 = bd.detach().cpu().numpy()[i]
                            # bd1 = ((bd1 - bd1.min()) / (bd1.max() - bd1.min()) * 255).astype(np.uint8)
                            # tifffile.imwrite(fr"D:\omnipose-main\results3d\{i + 1}\bd{i + 1}.tif", bd1)

                            T1 = T.detach().cpu().numpy()[i]
                            T1 = ((T1 - T1.min()) / (T1.max() - T1.min()) * 255).astype(np.uint8)
                            tifffile.imwrite(fr"D:\omnipose-main\results3d\{i+1}\dist{i+1}.tif", T1)

                            # mu1 = mu.detach().cpu().numpy()[i]
                            # # mu1 = np.transpose(mu1, (1, 2, 3, 0))
                            # tifffile.imwrite(fr"D:\omnipose-main\results3d\{i+1}\flow{i+1}.tif", mu1)
                            # tifffile.imwrite(fr"D:\omnipose-main\results3d\{i+1}\flow_color{i+1}.tif",
                            #                  plot.dx_to_circ(mu1, transparency=True)
                            #                  if self.nclasses > 1 else np.zeros(
                            #                      T1.shape + (3 + True,), np.uint8)
                            #                  )
                    torch.cuda.empty_cache()
                    # lbl[:,0] cell masks——Tag
                    # lbl[:,1] thresholded mask layer——阈值掩码层,
                    # lbl[:,2] boundary field——边界场 bd,
                    # lbl[:,3] smooth distance field ——平滑距离场 dist,
                    # lbl[:,4] boundary-emphasizing weights——均方误差加权,
                    # lbl[:,5:] flow components ——Stream
                    # batch labels does all the final assembling of the prediction classes
                    ...
                    #                                  nclasses=self.nclasses,
                    #                                  device=self.device)
                    lbl = omnipose.core.batch_labels2(masks, T, tyx,
                                                      dim=self.dim,
                                                      nclasses=self.nclasses,
                                                      device=self.device)
                    dt = time.time()-tic
                    ...
                    train_loss = self._train_step2(self._to_device(np.stack(imgi)), lbl)
                    # 在第二个维度插入大小为 1 的维度
                    # train_loss = self._train_step(self._to_device(np.expand_dims(lbl[:, 3], 1)), lbl)
                    # train_loss = self._train_step(torch.unsqueeze(lbl[:, 3], 1), lbl)
                    ...
                    lsum += train_loss
                    nsum += nbatch
                    # with open(os.path.join(file_path, "train2.txt"), "a") as f:
                    #     f.write("{} train_loss:{}\n".format(iepoch, train_loss / nbatch))
            tic = time.time()
            ...
            self.net.eval()
            with torch.no_grad():
                for vi in range(len(val_images)):
                    # 计算评估指标
                    im = np.transpose(val_images[vi], (1, 2, 3, 0))
                    y, style = self._run_net(im,  # Output
                                             augment=False,
                                             tile=0,
                                             normalize=True,
                                             tile_overlap=0.1,
                    ...
                    pred_dist = y[..., 0]
                    seg = y[..., 1]
                    pred_dist = self._to_device(pred_dist)
                    seg = self._to_device(seg)
                    seg = torch.unsqueeze(seg, 0)
                    eval0 = 1 - self.criterion12(pred_dist, val_distances[vi], val_weights[vi])
                    eval = eval0 + eval_metric([], seg, val_masks[vi])
                    # eval = eval_metric(seg, val_masks[vi])
                    # eval = 1 - self.criterion2(seg.float(), val_masks[vi][0]) / 20
                    # curEvalVal += eval
                    # 评估指标
                    evalVal.update(eval)
                # curEvalVal /= len(val_images)
                curEvalVal = evalVal.update2()
                curEvalVal = curEvalVal
                if save_path is not None:
                    if self.torch and self.gpu:
                        if curEvalVal > lastEvalVal:
                            self.net.module.save_model(os.path.join(file_path, "myotorch_1"))
            ...
            with torch.no_grad():
                if save_path is not None:
                    if self.torch and self.gpu and fore_loss > t_loss:
                        self.net.module.save_model(os.path.join(file_path, "myotorch_0"))
                        fore_loss = t_loss
            try:
                with open(os.path.join(file_path, "train.txt"), "w") as f:
                    f.write(("Train epoch: {}\n"
                             "Time: {:.2f}min\n"
                             "last epoch: {:.2f}s\n"
                             # "batch size: {} | "
                        ...
                        np.mean(datatime[-3:]),
                        # average time spent loading fata for last 3 epochs
...
class LearningRateScheduler:
    def __init__(self, initial_learning_rate, n_epochs, step_size=100, gamma=0.5):
    ...
    def get_learning_rate(self, epoch):
        # 计算当前epoch对应的学习率
        return self.initial_learning_rate * (self.gamma ** (epoch // self.step_size))

    def generate_learning_rates(self):
        # 生成整个训练过程中的学习率数组
        LR = np.array([self.get_learning_rate(epoch) for epoch in range(self.n_epochs)], dtype=np.float32)
        return LR
