...
def masks_to_flows_batch(batch, links=[None], device=torch.device('cpu'), 
                         ...
                         verbose=False):
    """
    batch：分割掩码的列表，每个掩码的形状为 [tyx]
    links：标签之间的连接关系，用于处理多标签对象
    device：计算设备，默认为 torch.device('cpu')
    omni：是否使用 OmniPose 模式
    dim：数据的维度，默认为 2D
    smooth：是否对流场进行平滑处理
    normalize：是否对流场进行归一化
    affinity_field：是否计算亲和场
    initialize：是否初始化流场
    n_iter：迭代次数
    verbose：是否打印调试信息
    ...
    """   
    # 将多个分割掩码合并为一个大数组，并计算它们的边界和流场
    # 通过计算亲和图（affinity graph）和边界，生成用于训练的流场
    # 包括平滑、Normalization、初始化等
    # 返回处理后的标签、Boundary、流场、切片索引等信息
    # add an if statement to catch the case where all labels are empty 
    
    nsample = len(batch)
    # 将所有分割掩码合并为一个大数组，同时返回合并后的标签、连接关系、索引和形状
    # final_flat：合并后的标签数组（展平）
    # clinks：合并后的连接关系
    # indices：索引数组
    # final_shape：合并后的数组形状
    # dL：每个样本的边界偏移量
    ...
    # 计算欧几里得距离场
    import eikonal
    # 使用 unsqueeze 在第 2 个维度插入一个大小为 1 的新维度
    mask = np.expand_dims(batch, axis=1)
    ...
    # ccoords：坐标索引
    # affinity_graph：亲和图
    return torch.tensor(clabels.astype(int),device=device), torch.tensor(boundaries,device=device), distance, gradient, slices, clinks, ccoords, affinity_graph


def masks_to_flows_batch2(batch, links=[None], device=torch.device('cpu'),
                         omni=True, dim=2, smooth=False, normalize=False,
                         affinity_field=False, initialize=False, n_iter=None,
                         verbose=False):
    """
    ...
    # final_shape：合并后的数组形状
    # dL：每个样本的边界偏移量
    # final_flat, final_shape, dL = concatenate_labels2(batch, links=links, nsample=nsample)
    # clabels = final_flat.reshape(final_shape)
    ...
    # ccoords：坐标索引
    # affinity_graph：亲和图
    # X = torch.tensor(clabels.astype(int), device=device)
    # masks = torch.stack([X[(Ellipsis,) + slc] for slc in slices])
    return torch.tensor(batch, device=device), distance
...
def concatenate_labels(masks: np.ndarray, links: list, nsample: int):
    ...
    for i, masks in enumerate(zip(masks)):
...
def concatenate_labels2(masks: np.ndarray, links: list, nsample: int):
    # concatenate and increment both the masks and links
    masks = masks.copy().astype(np.int64)  # casting to int64 sped things up 10x???
    dtype = masks[0].dtype
    shape = masks[0].shape
    ...
    return final_flat, final_shape, dL


# LABELS ARE NOW (masks,mask) for semantic seg with additional (bd,dist,weight,flows) for instance seg
# semantic seg label transformations taken care of above, those are simple enough.
# Others must be computed after mask transformations are made. Note that some of the labels are NOT used in training.
# Masks are never used, and boundary field is conditionally used.
def batch_labels(masks,bd,T,mu,tyx,dim,nclasses,device,dist_bg=5):
    ...
    lbl[:,0] = masks # probably do not need to store this here, but will keep it for now ——Tag，掩码
    lbl[:,1] = lbl[:,0]>0 # used to interpolate the mask, now thinking it is better to stay perfectly consistent ——阈值掩码层
    
    if nt>2:
        lbl[:,2] = bd # posisiton 2 store boundary, now returned as part of linked flow computation  ——Boundary
        lbl[:,3] = T # position 3 stores the smooth distance field ——距离场
        # lbl[:,3] = torch.log(lbl[:,3]+5) # try to reduce impact of large values 
        lbl[:,3][lbl[:,3]<=0] = -dist_bg # balance with boundary logits ——平滑边界逻辑
        
        lbl[:,-dim:] = mu*5.0 # *5 puts this in the same range as boundary logits
        lbl[:,4] = (1+lbl[:,1])/2 # position 4 stores the weighting image for weighted MSE ——均方误差加权
...
def batch_labels2(masks, T, tyx, dim, nclasses, device, dist_bg=5):
    nimg = len(masks)

    nt = 2  # instance seg (labels), semantic seg (cellprob)
    if nclasses > 1:
        nt += 2  # add distance, weight
    ...
    if nt > 2:
        lbl[:, 2] = T  # position 2 stores the smooth distance field ——距离场
        lbl[:, 2][lbl[:, 2] <= 0] = -dist_bg  # balance with boundary logits ——平滑边界逻辑
        lbl[:, 3] = (1 + lbl[:, 1]) / 2  # position 3 stores the weighting image for weighted MSE ——均方误差加权
    return lbl
...
def mode_filter(masks):
    ...
    """
    # 快速的众数滤波器（mode filter），用于清理插值后的标签
    # masks 是输入的多维数组，表示需要进行滤波的标签
    # 使用 np.pad 对 masks 进行边界填充，填充宽度为 pad=1。填充的目的是为了处理边缘像素的邻域计算问题，避免边界溢出
    # 将 masks 转换为整数类型，方便后续计算
    pad = 1
    masks = np.pad(masks, pad).astype(int)
    d = masks.ndim  # 是输入数组的维度
    shape = masks.shape  # 是输入数组的形状
    # coords 是一个元组，包含 masks 中非零元素的坐标。例如，对于二维数组，coords[0] 是行索引，coords[1] 是列索引
    coords = np.nonzero(masks)  # 是非零点的坐标
    steps, inds, idx, fact, sign = utils.kernel_setup(d)
    
    # subinds = np.concatenate(inds[0:2]) # only consider center+cardinal 
    subinds = np.concatenate(inds)
    substeps = steps[subinds]  # 是邻域的步长，定义了邻域的范围
    ...
    # 使用 neighbors 从 masks 中提取每个非零点的邻域值
    # neighbor_masks 的形状是 (n_neighbors, n_nonzero)，其中 n_neighbors 是邻域的大小，n_nonzero 是非零点的数量
    ...
    # 如果某个非零点的邻域众数为零，则将其替换为原始值。这一步是为了避免将非零点错误地归为零
    z = most_f==0 
    most_f[z] = masks[coords][z]
    # 将计算得到的众数值赋值到滤波后的数组 mask_filt 中
    mask_filt[coords] = most_f
    # 使用 unpad 去除之前添加的边界填充，恢复原始数组的大小
    unpad = tuple([slice(pad, -pad)]*d)
...
def _extend_centers_torch(masks, centers, affinity_graph, coords=None, n_iter=200, 
    ...
    if return_flows:
        ...
        mu_[(Ellipsis,)+coords] = _gradient(T,d,steps,fact,inds,isneigh,neigh_inds,central_inds,s)  # 计算流场
        if verbose:
            print('mu',mu_.shape)
...
def _gradient(T,d,steps,fact,
    ...
    finite_differences = torch.zeros(s,device=T.device,dtype=T.dtype)  # 初始化有限差分为0
...
def compute_masks(dP, dist, affinity_graph=None, bd=None, p=None, coords=None, iscell=None, niter=None, rescale=1.0, resize=None, 
    ...
    # compute_masks(dP, dist, bd, niter=10, resize=(192,192,192), mask_threshold=-5, flow_threshold=0, min_size=15, omni=False, verbose=1, use_gpu=self.gpu, device, nclasses, dim, flow_factor=10)
    """
...
def random_rotate_and_resize(X, Y=None, scale_range=1., gamma_range=[.75, 2.5], tyx = (224,224),
    ...
    # 定义两个向量 v1 和 v2，用于定义旋转平面。这些向量在 2D 和 3D 中的定义略有不同
    v1 = [0]*(dim-1)+[1]
    v2 = [0]*(dim-2)+[1, 0]
    # 遍历每张图像
    for n in range(nimg):
        ...
        # 通过随机裁剪、Rotate、缩放和增强操作，生成用于深度学习训练的多样化图像和标签
...
def random_crop_warp(img, Y, tyx, v1, v2, nchan, rescale, scale_range, gamma_range, do_flip, ind, 
    ...
    # 如果裁剪后的标签中唯一值少于 2（即没有足够的细胞像素），则递归重新裁剪
...
def loss(self, lbl, y):
    ...
    lbl: ND-array, float
        ...
        lbl[:,2] boundary field——边界场
        lbl[:,3] smooth distance field ——平滑距离场
    ...
    if self.nclasses == 1:  # semantic segmentation, generalize to logits
        ...
        # 流动场的准确性（flow_mse）。
        # 流动场的平滑性和物理合理性（SSL、norm_loss、lossE、lossDC）。
        # 边界场的准确性（bd_loss、lossB）。
        # 距离场的准确性（dist_loss）。
        # 流动场的连贯性（lossA）。
...
def loss2(self, lbl, y, X):
    """
    Loss function combining distance field loss and label (cell mask) loss.

    Parameters
    --------------
    lbl: ND-array, float
        transformed labels in array [nimg x nchan x xy[0] x xy[1]]
        lbl[:, 0] mask ——阈值掩码层（Tag）
        lbl[:, 1] thresholded mask layer ——阈值掩码层（用于标签损失）
        lbl[:, 2] smooth distance field ——平滑距离场（用于距离场损失）
        lbl[:, 3] boundary-emphasizing weights ——边界强调权重（用于加权损失）

    y:  ND-tensor, float
        network predictions, with dimension D, these are:
        y[:, 0] predicted distance field ——预测的距离场
        y[:, 1] predicted cell mask ——预测的细胞掩码
    """

    # 提取细胞掩码
    cellmask = lbl[:, 1] > 0  # 从阈值掩码层生成细胞掩码
    # cm = lbl[:, 0] / 255.
    cm = cellmask.float()  # 将细胞掩码转换为浮点数
    # 提取目标距离场
    dist = lbl[:, 2]  # 目标距离场
    # 提取权重
    w = lbl[:, 3].detach()  # Weight，用于加权损失

    # 提取预测细胞掩码和预测距离场
    pred_dist = y[:, 0]  # 预测的距离场

    # 计算距离场损失（加权均方误差）
    dist_loss = self.criterion12(pred_dist, dist, w)  # 加权MSE

    pred_mask = y[:, 1]  # 预测的细胞掩码
    # 计算标签损失（细胞掩码损失）
    # 均方误差（MSE）Loss
    # label_mse_loss = self.criterion(pred_mask, cm)
    # label_mse_loss = self.criterion12(pred_mask, cm, w)  # 加权MSE
    # 二值交叉熵损失（BCElogits）
    # label_bce_loss = self.criterion2(pred_mask.float(), cm)
    # 总标签损失
    # total_label_loss = label_mse_loss + label_bce_loss / 20

    cm = torch.unsqueeze(cm, 1)
    pred_mask = torch.unsqueeze(pred_mask, 1)
    l1loss = self.loss_criterion([], pred_mask, cm)[0]
    # SoftDice_loss = self.precise_dice_loss(cm, pred_mask)
    # BceDice_loss = self.bce_dice_loss(pred_mask, cm)
    # DiceFocalLoss = self.dice_focal_loss(pred_mask, cm)
    # diceLoss = self.dice_loss(pred_mask, cm)
    total_label_loss = l1loss

    # 总损失为标签损失和距离场损失的和
    total_loss = dist_loss + total_label_loss

    # 原始损失值，用于调试和分析
    raw_loss = total_loss.detach()

    # 返回总损失和原始损失
    return total_loss, raw_loss

    # total_loss = [label_mse_loss, label_bce_loss / 20, dist_loss]
    # total_loss = [dist_loss, total_label_loss]
    # raw_loss = sum(total_loss).detach() / len(total_loss)
    # # 返回总损失和原始损失
    # return sum(total_loss), raw_loss
