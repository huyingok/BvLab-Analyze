...
def get_neighbors(coords, steps, dim, shape, edges=None, pad=0):
    ...
    """
    # coords：非零点的坐标，是一个元组，每个元素是一个数组，表示在每个维度上的坐标
    # 邻域的步长，定义了邻域的范围，是一个数组，每行表示一个邻域偏移量
    # 输入数组的维度
    # 输入数组的形状
    # 边界填充宽度
    # 边界坐标，用于处理边界情况，如果为 None，则默认为 [-1+pad, s-pad]
    if edges is None:
        # 如果 edges 为 None，则根据 pad 和 shape 计算边界坐标
        edges = [np.array([-1+pad, s-pad]) for s in shape]
    # npix 是非零点的数量
    npix = coords[0].shape[-1]
    # nsteps 是邻域的大小
