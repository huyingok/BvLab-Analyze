...
class VesselDataSetMake(object):
    def __init__(self, win):
        ...
        # Predict
        self.VesselDataPredictQThread = VesselDataPredictQThread(win=self, source="make")
    ...
    def data_filter_error(self, e):
        self.mess(f"Vessel dataset filtering error: {e}", "Prompt", 1)
    ...
    def data_set_finish(self):
        ...
        if self.select_input_index == 1:
            ...
            self.mess(f"Vessel dataset making finished!", "Prompt", 1)
    ...
    def data_predict_finish(self, new_text):
        ...
        if new_text:
            self.mess(f"{new_text}, cannot make vessel dataset (predict)!", "Prompt", 1)
        else:
            self.mess(f"Vessel dataset making finished!", "Prompt", 1)

    def data_predict_error(self, e):
        self.mess(f"Vessel dataset making error: {e}", "Prompt", 1)
        self.recover_control()
    ...
    def data_cut_error(self, e):
        self.mess(f"Vessel image cutting error: {e}", "Prompt", 1)
        self.recover_control()
        ...
        try:
            if look and (self.cutting or self.filtering or self.making or self.predicting):
                ...
                if save:
                    ...
                    initial_path = settings.value("VesselMakePath", "")
                # Get folder path
                ...
                if path:
                    if not self.contains_chinese(path):
                        if save:
                            ...
                            settings.setValue("VesselMakePath", path)
    ...
    def select_path(self):
        try:
            ...
            settings = QSettings("MyCompany", "MyApp")
            # Read previously saved path
            initial_path = settings.value("VesselMakeModelPath", "")
            # Get file path
            ...
            if filename:
                if not self.contains_chinese(filename):
                    settings.setValue("VesselMakeModelPath", str(Path(filename).parent))
