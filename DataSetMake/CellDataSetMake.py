...
from CellUnet3D_DDP.PredictQThread import CellDataPredictQThread
from ImageCut.CellImageCutThread import CellImageCutThread
...
class CellDataSetMake(object):
    def __init__(self, win):
        ...
        """Data making"""
        # self.default_model = join(self.win.default_models_Dir, "Cell_Unet3D_Model0.pth")  # Built-in model
        ...
        # Predict
        self.CellDataPredictQThread = CellDataPredictQThread(win=self, source="make")
        self.CellDataPredictQThread.progress0.connect(self.data_predict_progress)
        self.CellDataPredictQThread.progress_text.connect(self.data_predict_progress_text)
        self.CellDataPredictQThread.finish0.connect(self.data_predict_finish)
        self.CellDataPredictQThread.error0.connect(self.data_predict_error)
        # Cut
        self.CellImageCutThread = CellImageCutThread(win=self)
        self.CellImageCutThread.progress0.connect(self.data_cut_progress)
        self.CellImageCutThread.finish0.connect(self.data_cut_finish)
        self.CellImageCutThread.error0.connect(self.data_cut_error)
    ...
    def data_filter_error(self, e):
        self.mess(f"Cell dataset filtering error: {e}", "Prompt", 1)
    ...
    def data_set_finish(self):
        ...
        if (self.select_input_index == 0 or
            ...
            self.CellDataPredictQThread.start()
        if self.select_input_index == 1:
            self.recover_control()
            ...
            self.mess(f"Cell dataset making finished!", "Prompt", 1)
    ...
    def data_predict_finish(self, new_text):
        ...
        if new_text:
            self.mess(f"{new_text}, cannot make cell dataset (predict)!", "Prompt", 1)
        else:
            self.mess(f"{new_text}   Cell dataset making finished!", "Prompt", 1)

    def data_predict_error(self, e):
        self.mess(f"Cell dataset making error: {e}", "Prompt", 1)
        self.recover_control()
    ...
    def data_cut_error(self, e):
        self.mess(f"Cell image cutting error: {e}", "Prompt", 1)
        self.recover_control()
        ...
        try:
            if look and (self.cutting or self.filtering or self.making or self.predicting):
                ...
                if save:
                    ...
                    initial_path = settings.value("CellMakePath", "")
                # Get folder path
                ...
                if path:
                    if not self.contains_chinese(path):
                        if save:
                            ...
                            settings.setValue("CellMakePath", path)
    ...
    def select_path(self):
        try:
            ...
            settings = QSettings("MyCompany", "MyApp")
            # Read previously saved path
            initial_path = settings.value("CellMakeModelPath", "")
            # Get file path
            ...
            if filename:
                if not self.contains_chinese(filename):
                    settings.setValue("CellMakeModelPath", str(Path(filename).parent))
