...
from ImageSplice.CellImgSpliceQThread import ImgSpliceQThread
...
class CellDataPredict(object):
    def __init__(self, win):
        ...
        # Thread
        self.CellDataPredictQThread = CellDataPredictQThread(win=self, source="pred")
    ...
    def img_splice_finish(self, text):
        ...
        self.mess(f"Cell image splicing finished! Time elapsed: {t:.4f} Minute", "Prompt", 1)

    def img_splice_error(self, e):
        self.splice_recover_control()
        self.mess(f"Cell image splicing error: {e}", "Prompt", 1)
    ...
    def data_predict_finish(self, new_text):
        ...
        t = (time.time() - self.start_time) / 60
        if new_text:
            self.mess(f"{new_text}, cannot predict cell data!", "Prompt", 1)
        else:
            self.mess(f"Cell data prediction finished! Time elapsed: {t:.4f} Minute", "Prompt", 1)
        self.splice_cfg_lineEdit.setText(self.cut_infos_path)

    def data_predict_error(self, e):
        self.recover_control()
        self.mess(f"Cell data prediction error: {e}", "Prompt", 1)
        ...
        try:
            if look and self.predicting:
                ...
                if save:
                    ...
                    initial_path = settings.value("CellPredictPath", "")
                ...
                if path:
                    if not self.contains_chinese(path):
                        if save:
                            ...
                            settings.setValue("CellPredictPath", path)
    ...
    def select_path(self, se):
        try:
            ...
            # Get file path
            if se == 0:
                initial_path = settings.value("CellPredictConfigPath", "")
            ...
            if se == 1:
                initial_path = settings.value("CellPredictJsonPath", "")
                path = os.path.join(initial_path, "cut_infos.json")
                filename, filetype = QFileDialog.getOpenFileName(None, "Select file", path, "Json File (*.json)")
            if se == 2:
                initial_path = settings.value("CellPredictModelPath", "")
            ...
            if filename:
                if not self.contains_chinese(filename):
                    if se == 0:
                        settings.setValue("CellPredictConfigPath", str(Path(filename).parent))
                        self.predict_cfg_lineEdit.setText(filename)
                    if se == 1:
                        settings.setValue("CellPredictJsonPath", str(Path(filename).parent))
                        self.splice_cfg_lineEdit.setText(filename)
                    if se == 2:
                        settings.setValue("CellPredictModelPath", str(Path(filename).parent))
                        self.predict_cfg_lineEdit.setText(filename)
                else:
                    self.mess(f"Path contains Chinese characters or spaces", "Prompt", 1)
        except Exception as e:
