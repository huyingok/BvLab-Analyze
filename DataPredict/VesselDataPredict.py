...
from VesselUnet3D.PredictQThread import VesselDataPredictQThread
from ImageSplice.VesselImgSpliceQThread import ImgSpliceQThread
...
class VesselDataPredict(object):
    def __init__(self, win):
        ...
        # Thread
        self.VesselDataPredictQThread = VesselDataPredictQThread(win=self, source="pred")
        self.VesselDataPredictQThread.finish0.connect(self.data_predict_finish)
        self.VesselDataPredictQThread.progress0.connect(self.data_predict_progress)
        self.VesselDataPredictQThread.progress_text.connect(self.data_predict_progress_text)
        self.VesselDataPredictQThread.error0.connect(self.data_predict_error)
    ...
    def img_splice_finish(self, text):
        ...
        self.mess(f"Vessel image splicing finished! Time elapsed: {t:.4f} Second", "Prompt", 1)

    def img_splice_error(self, e):
        self.splice_recover_control()
        self.mess(f"Vessel image splicing error: {e}", "Prompt", 1)
            ...
            if not self.predicting:
                ...
                if bv_ROI_check:
                    ...
                    if (MaxX - MinX < small_size[0]
                        ...
                        self.mess(f"The minimum size (Max-Min) of the ROI region in BV format cannot be less than [x, y, z]={small_size}. Please reset the ROI range!",
                                  "Prompt", 1)
                        return
                    else:
                            ...
                            if os.path.isdir(image_dir):
                                if os.path.exists(model_path):
                                    ...
                                    if res == QMessageBox.Yes:
                                        ...
                                        self.VesselDataPredictQThread.start()
                                else:
    ...
    def data_predict_finish(self, new_text):
        ...
        if new_text:
            self.mess(f"{new_text}, cannot predict vessel data!", "Prompt", 1)
        else:
            self.mess(f"Vessel data prediction finished! Time elapsed: {t:.4f} Minute", "Prompt", 1)
    ...
    def data_predict_error(self, e):
        self.recover_control()
        self.mess(f"Vessel data prediction error: {e}", "Prompt", 1)
        ...
        try:
            if look and self.predicting:
                ...
                if save:
                    ...
                    initial_path = settings.value("VesselPredictPath", "")
                ...
                if path:
                    if not self.contains_chinese(path):
                        if save:
                            ...
                            settings.setValue("VesselPredictPath", path)
    ...
    def select_path(self, se):
        try:
            ...
            # Get file path
            if se == 0:
                initial_path = settings.value("VesselPredictConfigPath", "")
            ...
            if se == 1:
                initial_path = settings.value("VesselPredictJsonPath", "")
                path = os.path.join(initial_path, "cut_infos.json")
                filename, filetype = QFileDialog.getOpenFileName(None, "Select file", path, "Json File (*.json)")
            if se == 2:
                initial_path = settings.value("VesselPredictModelPath", "")
            ...
            if filename:
                if not self.contains_chinese(filename):
                    if se == 0:
                        settings.setValue("VesselPredictConfigPath", str(Path(filename).parent))
                        self.predict_cfg_lineEdit.setText(filename)
                    if se == 1:
                        settings.setValue("VesselPredictJsonPath", str(Path(filename).parent))
                        self.splice_cfg_lineEdit.setText(filename)
                    if se == 2:
                        settings.setValue("VesselPredictModelPath", str(Path(filename).parent))
                        self.predict_cfg_lineEdit.setText(filename)
                else:
                    self.mess(f"Path contains Chinese characters or spaces", "Prompt", 1)
        except Exception as e:
