...
class ImgSpliceQThread(QThread):
    ...
    def ImgSplice_Whole(self, 
        ...
        del swcData
        swc_add_ = swc_add
        return swc_add, swc_add_, [nz * br_z, ny * br_y, nx * br_x]
    ...
    def bv_swc_splice_sub(self,
        ...
        seg_work_dir = join(save_dir, "seg_work_dir")
        if os.path.exists(seg_work_dir):
        ...
        for nx in range(BvSliceNumberXYZ[0]):
            for ny in range(BvSliceNumberXYZ[1]):
                for nz in range(BvSliceNumberXYZ[2]):
                    ...
                    bv_seg_path = join(seg_work_dir, bv_seg_name)
                ...
                if len(names) == len(files_names):
                    ...
                    if len(add_list) > 0:
                        for files_name in add_list:
                            ...
                            cut_seg_dir = bv_cut_info.get("cut_seg_dir", None)
        ...
        # # 预分配大图数组，初始化为 0 (背景)
        # big_array = np.zeros((BvBigSizeXYZ[2] + 1,
        #                       BvBigSizeXYZ[1] + 1,
        #                       BvBigSizeXYZ[0] + 1), dtype=np.uint8)
        ...
        for nx in range(BvSliceNumberXYZ[0]):
            for ny in range(BvSliceNumberXYZ[1]):
                for nz in range(BvSliceNumberXYZ[2]):
                    ...
                    swc_add, swc_add_, [start_z, start_y, start_x] = self.ImgSplice_Whole(bigSize=BvBigSizeXYZ,
                                                                                          ...
                                                                                          lv=lv,
                                                                                          ROI=ROI)
                    # if os.path.exists(bv_seg_path):
                    #     # 读取小块的 TIF 分割结果
                    #     block_data = tifffile.imread(bv_seg_path)
                    #     block_shapes = block_data.shape
                    #     print(block_shapes)
                    #     # 将小块数据写入大图对应位置
                    #     try:
                    #         big_array[start_z:start_z+block_shapes[0],
                    #                   start_y:start_y+block_shapes[1],
                    #                   start_x:start_x+block_shapes[2]] = block_data
                    #     except:
                    #         print("Exceeding the maximum size!")
                    #         big_array_shapes = big_array.shape
                    #         big_array[
                    #         big_array_shapes[0]-block_shapes[0]:big_array_shapes[0],
                    #         big_array_shapes[1]-block_shapes[1]:big_array_shapes[1],
                    #         big_array_shapes[2]-block_shapes[2]:big_array_shapes[2]] = block_data
                    #     del block_data
        ...
        # big_array = big_array[:BvBigSizeXYZ[2], :BvBigSizeXYZ[1], :BvBigSizeXYZ[0]]
        # tifffile.imwrite(join(save_dir, "seg_splice.tif"), big_array, compression="lzw")
        # del big_array
...
def swc_splice_process(bigSize=None,
    ...
    for nz in range(sliceNumber[2]):  # Process Z axis
        for ny in range(sliceNumber[1]):  # Process Y axis
            for nx in range(sliceNumber[0]):  # Process X axis
                ...
                swc_path = join(cut_swc_dir, swc_name)
                if cut_seg_dir is not None:
                    seg_name = f"{str(nz).zfill(4)}_{str(ny).zfill(2)}_{str(nx).zfill(2)}.tif"
                    seg_path = join(cut_seg_dir, seg_name)
    ...
    if cut_seg_dir is not None:
        big_array = big_array[:Max_z, :Max_y, :Max_x]
        # 一次性写入大图
        tifffile.imwrite(bv_seg_path, big_array, compression="lzw")
        del big_array
