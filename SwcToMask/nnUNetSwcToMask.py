...
class nnUNetSwcToMaskQThread(QThread):
    finish0 = pyqtSignal()
    ...
    def __init__(self, *args, **kwargs):
        super(nnUNetSwcToMaskQThread, self).__init__()
    ...
    def run(self):
        try:
            if not self.win.train_stop:
                images_dir = self.win.data_train_dict['images_dir']
                SWC_dir = self.win.data_train_dict['SWC_dir']
                images_type = self.win.data_train_dict['images_type']
                self.continue_training = self.win.data_train_dict['continue_training']

                names = [l for l in os.listdir(images_dir) if ".tif" in l]

                if len(names) == 0:
                    self.warning0.emit(f"There are no TIF format files in the folder {images_dir}")
                    return

                if len([l for l in os.listdir(SWC_dir) if ".swc" in l]) == 0:
                    self.warning0.emit(f"There are no SWC format files in the folder {SWC_dir}")
                    return
                ...
                shapes_list = []
                for name in names[:5]:
                    shapes_list.append(tifffile.imread(join(images_dir, name)).shape)
                ...
                mask_dir = join(self.rootPath, 'mask')
                # 更新标签路径
                self.win.data_train_dict['mask_dir'] = mask_dir

                self.swc_to_mask_sub(images_dir, SWC_dir, mask_dir, names, images_type)
    ...
    def swc_to_mask_sub(self, images_dir, SWC_dir, mask_dir, names, images_type):
        kernelLen = 3
        th = 103
        if images_type == "cell":
            kernelLen = 10
        elif images_type == "neuron":
            th = 73
        elif images_type == "vessel":
            th = 13
        ...
        for i, name in enumerate(names):
            mask_path = join(str(mask_dir), name)
            if self.continue_training:
                if os.path.exists(mask_path):
                    continue

            img_path = join(str(images_dir), name)
            ...
            pathLsQue.put((swc_path, mask_path, img_path, kernelLen, maxD, minD, th))
        ...
        for i in range(nums_work):
            if images_type == "cell":
                ps.append(Process(target=cell_swc_to_mask_process, args=(pathLsQue, finishQue, errorQue, resultQue)))
            elif images_type == "neuron":
                ps.append(Process(target=line_swc_to_mask_process, args=(pathLsQue, finishQue, errorQue, resultQue)))
            elif images_type == "vessel":
...
def cell_swc_to_mask_process(pathLsQue, finishQue, errorQue, resultQue):
    """处理单个图像，用于并行处理"""
    while True:
        try:
            ...
            swc_path, mask_save_path, img_path, kernelLen, maxD, minD, th = add

            img = tifffile.imread(img_path)
            imgShape = img.shape[::-1]  # 转为xyz
...
def line_swc_to_mask_process(pathLsQue, finishQue, errorQue, resultQue):
    """处理单个图像，用于并行处理"""
    while True:
        try:
            add = pathLsQue.get(timeout=5)
...
def swc_to_mask_from_radii_sub(pathLsQue, finishQue, errorQue, resultQue):
    ...
    while True:
        try:
            ...
            swc_path, save_path, img_path, kernelLen, maxD, minD, th = add
            ...
            # 二值化
            dfImg_add2[dfImg_add2 < th] = 0
            dfImg_add2[dfImg_add2 > 0] = 1
