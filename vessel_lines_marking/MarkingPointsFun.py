...
from vessel_lines_marking.ThreeLineSwcCurve import ThreeCurveFitPc
from vessel_lines_marking.ControlStyle import button_style, button_alpha_style
...
class MarkingPointsFun:
    ...
    def _cross_line_points(cls, self, camera_pos, lp1, is_left):
        ...
        if len(intersection_points):
            ...
            points, count = self.collect_points(intersection_points[0], intersection_points[1])
            if count:
                # maxPoint, statistics_dict = self.points_pos_sure(points, lp1)
    ...
    def _points_pos_sure(cls, points, self):
        ...
        if self.img3d.dtype == np.uint8:
            ...
            max_th = min(65535, self.gray_max)
        img[img < min_th] = 0
        ...
        for i in range(filter_output_nums):
            ...
            point = np.array(point).astype(np.int32)  # xyz
            grayValue = img[point[2], point[1], point[0]]
        ...
        if self.function_index == 0:
            ...
            for i in range(filter_output_nums):
                if 0 < pi - i <= max_nums:
                    ...
                    ii += 1
                elif 0 < i - pi <= max_nums:
                    index_list.append(ii)
                    ii += 1
                elif pi - i == 0:
                    statistics_dict['maxIndex'] = ii
                    index_list.append(ii)
                    ii += 1
            statistics_dict['index_list'] = index_list
            statistics_dict['point_list'] = [point_list[i] for i in range(filter_output_nums) if abs(pi - i) <= max_nums]
            statistics_dict['gray_list'] = [gray_list[i] for i in range(filter_output_nums) if abs(pi - i) <= max_nums]
