...
def HistFiterZarr(HistDataPath, HistAnalyPath, n_clusters, getClsNum):
...
'''对指定 batch 块列表提取直方图特征，结果写入 HistDataPath。

threIdOffset 用于分批处理时保证不同批次进程保存的文件名不冲突。
'''


def ExtractHistFromBatchIds(zarr_root, HistDataPath, nameIdLs, batchSize, smallSize, redunSize, level, MNumber,
    ...
    ps = []
    for i in range(MNumber):
        ps.append(Process(target=SignZarrToHist, args=(workQue, finishQue, errorQue, HistDataPath, threIdOffset + i,
                                                       zarr_root, batchSize, smallSize, redunSize, level)))
        ps[-1].daemon = True
        ps[-1].start()
    sTime = time.time()
...
'''根据 ImgIdLs.json 提取 tif 图像，写入 savePath。'''


def ExtractImgsByImgIdLs(ImgIdLsPath, zarr_root, savePath, batchSize, smallSize, redunSize, level, MNumber,
    ...
    ps = []
    for i in range(MNumber):
        ps.append(Process(target=SignZarrToTif, args=(zarr_root, savePath, batchSize, workQue, finishQue, errorQue,
                                                      smallSize, redunSize, level)))
        ps[-1].daemon = True
        ps[-1].start()
    sTime = time.time()
...
def start_zarr(divide_results_dir, img_dir, divide_img_dir, makerInfo,
    ...
    # 一次性提取全部 batch 块的特征
    try:
        ExtractHistFromBatchIds(zarr_root, HistDataPath, batchIdLs, batchSize, smallSize, redunSize, level, MNumber,
    ...
    HistAnalyPath = join(TrainDataSetFiterPath, "HistAnaly")
    ImgIdLsPath = HistFiterZarr(HistDataPath, HistAnalyPath, n_clusters, getClsNum)
    ...
    try:
        ExtractImgsByImgIdLs(ImgIdLsPath, zarr_root, divide_img_dir, batchSize, smallSize, redunSize, level, MNumber,
