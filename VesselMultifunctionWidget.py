...
import qdarkstyle
from UI.Ui_VesselMultifunctionWidget import Ui_VesselMultifunctionWidget
from DataSetMake.VesselDataSetMake import VesselDataSetMake
from DataPredict.VesselDataPredict import VesselDataPredict
from DataTrain.VesselDataTrain import VesselDataTrain
from DataStatistics.VesselStatistics import VesselDataStatistics
from ViewWidget.VesselViewWidget import VesselViewWidget
...
from DataStatistics.create_chart import MainWindow


class VesselMultifunctionWidget(QWidget, Ui_VesselMultifunctionWidget):
    def __init__(self, IntegrateWindow):
        ...
        # 模型日志路径
        logs_path = join("./logs", "vessel_logs")
        os.makedirs(logs_path, exist_ok=True)
        exe_cfg.cfg['ExePath']['vessel_logs_path'] = str(os.path.abspath(logs_path))

        # 模型保存路径
        models_path = join("./ModelSave", "VesselTrainModel")
        os.makedirs(models_path, exist_ok=True)
        exe_cfg.cfg['ExePath']['vessel_models_path'] = str(os.path.abspath(models_path))

        # Program Path
        exe_cfg.cfg['ExePath']['base_path'] = self.base_path

        # 端口，地址
        port = exe_cfg.cfg['Port']['vessel_port']
        exe_cfg.cfg['Port']['vessel_port'] = port
        host = exe_cfg.cfg['Host']['vessel_host']
        exe_cfg.cfg['Host']['vessel_host'] = host
        ...
        # 默认模型路径
        self.default_models_Dir = join(self.base_path, 'DefaultModels', 'VesselModels')
        ...
        # 修订
        self.VesselViewWidget = VesselViewWidget()
        self.revising_verticalLayout.addWidget(self.VesselViewWidget)

        self.VesselDataSetMake = VesselDataSetMake(self)  # 制作
        self.VesselDataTrain = VesselDataTrain(self)  # Train
        self.VesselDataPredict = VesselDataPredict(self)  # Predict
        self.VesselDataStatistics = VesselDataStatistics(self)  # Statistics
...
if __name__ == "__main__":
    ...
    # pyinstaller VesselMultifunctionWidget.py
    # pyinstaller --onefile CellMultifunctionWidget.py
    # pyinstaller VesselMultifunctionWidget.spec
    # pyinstaller VesselMultifunctionWidget_files.spec
    # pyinstaller VesselMultifunctionWidget_one.spec
    ...
    freeze_support()
    win = VesselMultifunctionWidget(None)
    # win.showMaximized()
    win.show()
    sys.exit(app.exec_())
