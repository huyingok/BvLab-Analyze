...
from DataStatistics.NeuralStatisticsQThread import NeuralStatisticsQThread
...
class NeuralDataStatistics(object):
    def __init__(self, win):
        ...
        self.NeuralStatisticsQThread = NeuralStatisticsQThread(win=self)
        self.NeuralStatisticsQThread.finish0.connect(self.statistics_finish)
        self.NeuralStatisticsQThread.progress_text.connect(self.statistics_progress)
        self.NeuralStatisticsQThread.error0.connect(self.statistics_error)
        self.NeuralStatisticsQThread.feature_files.connect(self.statistics_files)
        self.NeuralStatisticsQThread.chart_files.connect(self.statistics_chart)
            ...
            if not self.analyzing:
                ...
                if data_type == "bv":
                    ...
                    if res == QMessageBox.Yes:
                        ...
                        self.NeuralStatisticsQThread.start()
                else:
                    if img_path:
                        if swc_path:
    ...
    def create_chart(self, path, main_layout):
        ...
        self.plot_options = [
            ...
            "Segment Tortuosity Distribution Histogram",
        ]
    ...
    def _draw_single_plot(self, idx, ax):
        if idx == 0:
            self._draw_length_hist(ax)
        elif idx == 1:
            self._draw_tortuosity_hist(ax)

    def _draw_length_hist(self, ax):
        ax.hist(self.data['Length'], bins=30, color='skyblue', edgecolor='black', alpha=0.7)
        ax.set_title('Segment Length Distribution')
        ax.set_xlabel('Length')
        ax.set_ylabel('Frequency')
        ax.axvline(self.data['Length'].mean(), color='red', linestyle='--',
                   label=f'Mean Length: {self.data["Length"].mean():.2f}')
        ax.legend()

    def _draw_tortuosity_hist(self, ax):
        ax.hist(self.data['Tortuosity'], bins=30, color='lightgreen', edgecolor='black', alpha=0.7)
        ax.set_title('Segment Tortuosity Distribution')
        ax.set_xlabel('Tortuosity')
        ax.set_ylabel('Frequency')
        ax.axvline(self.data['Tortuosity'].mean(), color='blue', linestyle='--',
                   label=f'Mean Tortuosity: {self.data["Tortuosity"].mean():.2f}')
        ax.legend()
    ...
    def radius_calculate_finish(self, new_text):
        ...
        if new_text:
            self.mess(f"{new_text}, cannot perform neural data statistics!", "Prompt", 1)
        else:
            self.mess(f"Neural data statistics finished! Time elapsed: {t:.4f} Minute", "Prompt", 1)

    def radius_calculate_error(self, e):
        self.recover_control()
        self.mess(f"Neural data statistics error: {e}", "Prompt", 1)
        ...
        try:
            if look and self.analyzing:
                ...
                if save:
                    ...
                    initial_path = settings.value("NeuralAnalysisPath", "")
                ...
                if path:
                    if not self.contains_chinese(path):
                        if save:
                            ...
                            settings.setValue("NeuralAnalysisPath", path)
    ...
    def select_path(self, se):
        try:
            ...
            # Get file path
            if se == 1:
                initial_path = settings.value("NeuralAnalysisJsonPath", "")
            ...
            if filename:
                if not self.contains_chinese(filename):
                    if se == 1:
                        settings.setValue("NeuralAnalysisJsonPath", str(Path(filename).parent))
