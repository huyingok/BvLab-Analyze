...
class DataAnalyzer:
    ...
    def load_excel(self, filepath):
        ...
        # Basic Metric Columns：从 File Name 到 Mean Segment Tortuosity，通常是前8Columns
        # But Column Count May Change Due to Merged Cells，We Can Judge Based on Column Name Content
        basic_indicators = [
            'File Name', 'Network Length',
        ...
        for name in basic_indicators:
            ...
            if not found:
                ...
                # Simple Processing：取前8Columns
                basic_indices = list(range(min(8, len(col_names))))
                break
        ...
        # 如果少于8，Fill With Subsequent Columns？But May Cause Misalignment，Better Let User Choose
        if len(basic_indices) < 8:
            QMessageBox.warning(None, "Warning",
                                f"Only found {len(basic_indices)} basic metric columns, expected 8. Please check Excel format; some columns may be missing. Will process with existing columns.")
            # 补全到8Columns，用None占位
            while len(basic_indices) < 8:
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def load_file(self):
        ...
        initial_path = settings.value("NeuralXlsxPath", "")
        filepath, _ = QFileDialog.getOpenFileName(self,
                                                  "SelectExcelFile",
                                                  initial_path,
                                                  "ExcelFile (*.xlsx *.xls)")
        if filepath:
            settings.setValue("NeuralXlsxPath", str(Path(filepath).parent))
