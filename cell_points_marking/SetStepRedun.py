...
class SetStepRedun:
    ...
    def set_step(cls, step, redun, step_spinbox, redun_spinbox):
        ...
        if step <= redun:
            step = step_copy
        redun_spinbox.blockSignals(True)
        redun_up = int(step) - 1
        redun_spinbox.setRange(0, redun_up)
        redun_spinbox.blockSignals(False)
        return step
    ...
    def set_redun(cls, step, redun, step_spinbox, redun_spinbox, size):
        ...
        if step <= redun:
            redun = redun_copy
        step_spinbox.blockSignals(True)
        step_down = max(int(redun) + 1, int(size / 10) + 1)
        step_spinbox.setRange(step_down, int(size))
        step_spinbox.blockSignals(False)
        return redun
